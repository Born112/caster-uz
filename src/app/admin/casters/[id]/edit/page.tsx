...
export default async function EditCasterPage({ params }: { params: Params }) {
  ...
  const { data: caster, error } = await supabase
    .from("casters")
    .select("*")
    .eq("id", id)
    .single();
    ...
    <main className="min-h-screen bg-[#0A0E1A] text-white">
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex items-center gap-2 text-sm text-[#8B92A8] mb-4 mt-4">
          ...
          <span>/</span>
          <Link href="/admin/casters" className="hover:text-white transition-colors">
            Casterlar
          </Link>
          <span>/</span>
          <span className="text-white">{caster.nickname} - Tahrirlash</span>
        </div>

        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">
            <span className="text-[#00D9FF]">✏️</span> Tahrirlash:{" "}
            <span className="text-[#FF6B35]">{caster.nickname}</span>
          </h1>
          <p className="text-[#8B92A8]">Caster ma&apos;lumotlarini o&apos;zgartiring</p>
        </div>

        <CasterForm mode="edit" caster={caster} />
      </div>
    </main>
  );
}
