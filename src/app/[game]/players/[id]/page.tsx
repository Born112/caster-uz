...
export default async function PlayerProfilePage({ params }: { params: Params }) {
  ...
  const { data: player, error } = await supabase
    ...
    .select(`
      *,
      team_memberships(
        id, is_current, position, jersey_number, role, start_date, end_date,
        teams(id, name, short_name, game)
      ),
      national_memberships(
        id, is_current, position, role, start_date, end_date,
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="bg-gradient-to-br from-[#131929] to-[#0A0E1A] border border-white/10 rounded-2xl p-6 md:p-8 mb-6">
          <div className="flex flex-col md:flex-row items-start gap-6">
            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-[#0A0E1A] border-4 flex items-center justify-center font-bold text-3xl md:text-4xl shrink-0"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap mb-2">
                ...
                {player.is_verified && (
                  <span className="bg-green-500/20 text-green-400 text-xs font-bold px-2 py-1 rounded-md">
                    ✓ Tasdiqlangan
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 flex-wrap mb-4">
                <div
                  ...
                  style={{
                    backgroundColor: rank <= 3 ? gameInfo.color : "rgba(255,255,255,0.05)",
                    color: rank <= 3 ? "#0A0E1A" : "#FFFFFF",
                  }}
                >
              ...
              {player.telegram_username && (
                <a href={"https://t.me/" + player.telegram_username} target="_blank" rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {(currentClub || currentNationals.length > 0) && (
              <div className="bg-[#131929] border border-white/10 rounded-xl p-6">
                <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                  <span style={{ color: gameInfo.color }}>●</span> Hozirgi holat
                </h2>
            ...
            {player.bio && (
              <div className="bg-[#131929] border border-white/10 rounded-xl p-6">
                <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
                  <span style={{ color: gameInfo.color }}>●</span> Tavsif
                </h2>
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-[#131929] border border-white/10 rounded-xl p-6">
              <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                <span style={{ color: gameInfo.color }}>●</span> 📊 Statistika
              </h2>
              ...
              <div className="space-y-4 text-sm">
                ...
                <div>
                  ...
                  <div className="font-bold flex items-center gap-2">
                    {rankEmoji && <span className="text-xl">{rankEmoji}</span>}
                    <span>#{rank} <span className="text-[#8B92A8] text-xs">/ {totalPlayers}</span></span>
                  </div>
                </div>

                <div>
                  <div className="text-xs text-[#8B92A8] mb-1">Klub tarixi</div>
